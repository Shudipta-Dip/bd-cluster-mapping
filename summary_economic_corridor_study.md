# Summary – NW–Dhaka–SE Economic Corridor Study

> Source: *Summary - Economic Corridor Study.pdf* (April 2026, ADB)

## What It Is
- **Northwest–Dhaka–Southeast (NW–D–SE) Economic Corridor** initiative
- Covers **44 districts** within 50 km influence area on both sides of the transport spine
- Aligned with BNP's election manifesto → targets **$1 trillion economy by 2034**

## Why It Matters
- Manufacturing hyper-concentrated in Dhaka–Chattogram; rest of the country lags
- Corridor adds **2%+ annual GDP growth** over next decade
- **~10 million jobs** in 10 years
- **~12 million** lifted out of poverty in 3 years
- Corridor contributes **20%+** toward the trillion-dollar target

## Growth Nodes & Sector Specializations

| # | Growth Node | Key Sectors |
|---|---|---|
| 1 | **Moheshkhali** | Shipbuilding, ship repair/recycling, energy, steel & heavy engineering |
| 2 | **Chattogram** ★ | Port-centric logistics, medical tourism, RMG, leather manufacturing |
| 3 | **Mirsharai** | Diversified manufacturing (chemicals, electronics), IT/BPO, warehousing |
| 4 | **South Dhaka** | Mature EPZs (Adamjee 98%, Cumilla 95% occupied), pharmaceuticals, food processing |
| 5 | **North Dhaka** | G2G economic zones (Japanese EZ), IT, R&D, electronics (Hi-Tech City) |
| 6 | **Bogura–Rangpur–Thakurgaon** ★ | Agro-processing (potatoes, maize, jute), cold storage, light engineering, cross-border trade |
| 7 | **South-West** | Port-led manufacturing (Mongla Port), agro-processing, RMG/textiles |
| 8 | **Tourism** | Coastal tourism (Naf, Sabrang), eco-tourism, beach resorts |
| 9 | **North-East (Sylhet)** | Agro-processing, cross-border logistics (Tamabil, Sheola), IT/ITES, tourism |

> ★ = Recommended priority nodes (Chattogram and Bogura–Rangpur–Thakurgaon)

## Regional Strategy

| Region | Strategy |
|---|---|
| **Northwest** | Transform into agro-processing & cross-border trade hub (India, Nepal, Bhutan). Agricultural export zones in northern districts. Nationwide cold-storage networks. |
| **Central (Gazipur–Dhaka–Feni)** | High-value industrial & tech hub. Pivot to ICT — 1M tech jobs, semiconductor dev, AI data center campus. Decongest Dhaka via satellite towns. |
| **Southeast (Chattogram–Teknaf)** | Primary maritime & blue economy gateway. Chattogram as commercial capital. Blue economy authority, deep-sea fishing, green ship recycling, 1M eco-tourism jobs. |

## Quick-Win Priority Sectors
- Agribusiness
- Light engineering
- Creative industries (SMEs)
- Jute & leather products
- Pharmaceuticals
- Technology villages

## Investment Plan

| Horizon | Period | Amount | Share |
|---|---|---|---|
| Short-term | FY26–28 | $19.7B | 25% |
| Medium-term | FY29–35 | $46.1B | 58% |
| Long-term | FY36–45 | $13.9B | 17% |
| **Total** | | **~$79.6B** | |

## Sector-wise Investment Breakdown

| Sector | Share | Amount |
|---|---|---|
| Water & Climate | 28.8% | $22.9B |
| Energy & Power | 22.3% | $17.7B |
| Railway | 11.7% | $9.3B |
| Roads | 9.6% | $7.7B |
| Education & Health | 7.9% | $6.3B |
| Logistics & Multimodal | 4.9% | $3.9B |
| Ports & IWT | 4.5% | $3.5B |
| Digital | 3.6% | $2.9B |
| Urban Development | 2.1% | $1.7B |
| Agriculture & NR | 1.6% | $1.3B |

## Key Financing Details
- ADB committing **$5B over 5 years** + **$5B co-financing** from dev partners
- Green Climate Fund grants, carbon trading, Green Bangladesh Bonds (BDT 50,000 crore)
- PPPs heavily encouraged, especially for ports and standalone infra

---

# Summary – SW–Dhaka–NE Economic Corridor Study

> Source: *Bangladesh Economic Corridor Development Highlights* (August 2023, ADB). **Not from local PDF** — sourced from ADB/SASEC publications.

## What It Is
- **Southwest–Dhaka–Northeast (SW–D–NE) Economic Corridor** — a separate, earlier corridor proposal
- Covers **14 districts** from Khulna division → Dhaka → Sylhet & Mymensingh divisions
- Goal: decentralize industrial growth away from Dhaka–Chattogram into lagging SW & NE regions

## Growth Nodes & Sector Specializations

| # | Growth Node | Key Sectors |
|---|---|---|
| 1 | **Dhaka–Gazipur–Narsingdi** | RMG, Pharmaceuticals, Electronics, Leather & Footwear |
| 2 | **Khulna–Jashore** | Leather & Footwear, Frozen Fish/Shrimp, Jute |
| 3 | **Mymensingh–Netrokona** | Food Processing, Transport Equipment, Ceramics/Glass |
| 4 | **Sylhet–Habiganj** | Ceramics & Glass (local clay, glass sand, natural gas >4 tcf), Food Processing, Cross-border trade with NE India |
| 5 | **Sunamganj** | Raw material supply — fisheries, agro-products (haor area) |

## 12 Shortlisted Manufacturing Sectors
1. Food Processing
2. Leather & Footwear
3. Textiles & RMG
4. Jute
5. Non-metallic Minerals (Cement, Glass, Ceramics)
6. Shipbuilding
7. Furniture
8. Transport Equipment
9. Pharmaceuticals
10. Plastic Products
11. Rubber & Rubber Products
12. Electrical/Electronics & Heavy Machinery

## Trade Gateways

| Direction | Gateways |
|---|---|
| **Southwest** | Mongla seaport, Payra seaport, Benapole land port, Bhomra land port |
| **Northeast** | Tamabil land port, Akhaura land port, Bibirbazar land port, Nakugaon land port |

## Projected Economic Impact

| Metric | By 2035 | By 2050 |
|---|---|---|
| Corridor Earnings | $129B | $286B |
| Incremental vs BAU | +$53B | +$178B |
| Total Employment | — | 71.8M |
| Additional Jobs Created | 2.3M (by 2025) | 40.7M |

---

# BIDA FDI Heatmap: 11 Priority Sector Clusters

> Enriched with data from *Details - Economic Corridor Study.pdf* (Feb 2026) — cluster framework (Ch.4), RCA dynamics (Ch.5), sectoral linkages (SAM 2024), and freight analysis (Ch.7).

## 1. Textiles & Apparel
- **Cluster(s):** Chattogram Cluster, Mirsharai Cluster, South-West Cluster
- **Belt(s):** Central Belt (Gazipur–Savar–Narayanganj–Cumilla)
- **EZ/EPZs nearby:** Adamjee EPZ, Cumilla EPZ, Dhaka EPZ
- **Ports nearby:** Chattogram Port, Mongla Port, Benapole Land Port
- **RCA:** Men's suits +2.06, men's underwear +2.73, women's blouses +1.70; women's overcoats –1.11
- **Quick wins in:** Jute & leather products, RMG
- **Existing Operations:** Youngone Corporation (CEPZ, Chattogram), Jinyuan Chemical Industry (BSMSN, Mirsharai), operational garment factories in Jashore EPZ (South-West).
- **Recommendation(s):** Move toward technical textiles, design-intensive production, and branding to sustain long-term competitiveness.
- **Note:** Sector total linkage 28.95 (highest among manufacturing). Textiles remain low-complexity (ECI ~ –1.2 to –1.4).

## 2. Pharmaceuticals & Healthcare
- **Cluster(s):** South Dhaka Cluster, North Dhaka Cluster, Chattogram Cluster
- **Belt(s):** Central Belt
- **EZ/EPZs nearby:** Adamjee EPZ, Cumilla EPZ, Japanese Economic Zone (Sumitomo)
- **Ports nearby:** Chattogram Port
- **RCA:** High-complexity, high GVC-integration sector
- **Quick wins in:** Pharmaceuticals (generics), Medical tourism
- **Existing Operations:** Eskayef Pharmaceuticals (SK+F, Rupganj), Sun Pharma (Sonargaon), Beximco Pharma (Tongi), Albion Laboratories (Chattogram).
- **Recommendation(s):** Penetrate regional/global markets using TRIPS waivers and generic capacity.
- **Note:** Serves Dhaka's large consumer base; leverages proximity to Dhaka's universities and skilled labor pool.

## 3. Automotive & EV Ecosystem
- **Cluster(s):** Mymensingh–Netrokona, Mirsharai Cluster
- **Belt(s):** Central Belt
- **EZ/EPZs nearby:** Bangabandhu Sheikh Mujib Shilpa Nagar (BSMSN) in Mirsharai, Bangabandhu Hi-Tech Park (Gazipur), Abdul Monem EZ
- **Ports nearby:** Chattogram Port (approx. 60 km from BSMSN); Pangaon Inland Container Terminal (for Central Belt)
- **RCA:** Bicycles +0.20; machinery for preparing tobacco +1.68
- **Quick wins in:** Two- and three-wheeler EV manufacturing, auto parts assembly, import substitution of light vehicles
- **Existing Operations:** Runner Automobiles (operational motorcycle/EV assembly in Bhaluka, Mymensingh), Bangladesh Auto Industries Limited (BAIL) (Machinery installation operational in BSMSN, Mirsharai).
- **Recommendation(s):** Requires substantial FDI inflows and technology partnerships to develop. Prioritize resolving infrastructure bottlenecks (e.g., gas/utility connections for EV plants).
- **Note:** Motor vehicles sector total linkage 15.90. Local assembly is an emerging strategic growth area to reduce import dependency. Auto parts are emerging alongside electronics assembly.

## 4. Leather & Footwear
- **Cluster(s):** Chattogram Cluster, Khulna–Jashore
- **Belt(s):** Dhaka Industrial Belt
- **EZ/EPZs nearby:** Mature EPZs in Chattogram, Dhaka EPZ
- **Ports nearby:** Chattogram Port
- **RCA:** Raw hides +1.67, chamois leather +0.88, luggage/bags +0.11, leather apparel +0.11
- **Quick wins in:** Jute & leather products
- **Existing Operations:** Excelsior Shoes Ltd. (CEPZ, Chattogram), SAF Industries (LWG Gold-rated tannery in Noapara, Jashore).
- **Recommendation(s):** Address significant inefficiencies from fragmented last-mile connectivity and limited inland containerization.
- **Note:** Sector total linkage 22.66. Dense short-haul movements linked to tanneries, manufacturing clusters, and export consolidation points around Dhaka.

## 5. Electronics & Semiconductors
- **Cluster(s):** North Dhaka Cluster, Mirsharai Cluster
- **Belt(s):** Central Belt
- **EZ/EPZs nearby:** Japanese Economic Zone (Sumitomo), Hi-Tech City Kaliakair
- **Ports nearby:** Chattogram Port (approx. 60 km from Mirsharai); Hazrat Shahjalal International Airport (air freight for North Dhaka)
- **RCA:** Electrical equipment sector linkage still modest (total 7.13)
- **Quick wins in:** Consumer electronics assembly (mobile phones, home appliances), semiconductor design services
- **Existing Operations:** Walton Hi-Tech Industries (appliance assembly in Kaliakoir, Gazipur), McDonald Steel Building Products (operational in BSMSN, Mirsharai).
- **Recommendation(s):** Attract tech-intensive FDI to close the gap (product space shows electronics cluster largely empty vs. Vietnam). Support backward linkage for local component manufacturing.
- **Note:** Electronics is a key pivot sector. North Dhaka is positioned as Bangladesh's premier IT/R&D and electronics assembly hub. Sector projected to exceed $10 billion by 2030.

## 6. Plastics & Toys
- **Cluster(s):** South-West Cluster, South Dhaka Cluster
- **Belt(s):** Central Belt
- **EZ/EPZs nearby:** BSCIC Plastic Industrial Estate (Sirajdikhan, Munshiganj)
- **Ports nearby:** Mongla Port, Pangaon Inland Container Terminal
- **RCA:** Waste/parings/scrap of plastics +0.49
- **Quick wins in:** Export-oriented plastic toy manufacturing, plastic waste recycling, light plastic goods
- **Existing Operations:** Ideal Fibre Industries (Fatullah, Narayanganj), Diamond Disposable and Plastic Industries (Keraniganj), S.K. Plastic Industries (Siddhirganj).
- **Recommendation(s):** Expedite the completion of the BSCIC Plastic Industrial Estate (delayed to 2027) to relocate and formalize Old Dhaka's plastic factories. Integrate with light engineering for Mongla Port-linked manufacturing.
- **Note:** Plastics and chemicals sector has a total linkage of 17.42. Shortlisted manufacturing sector in both corridor studies.

## 7. Agro-processing
- **Cluster(s):** North Bengal Cluster, South-West Cluster, North-East (Sylhet)
- **Belt(s):** Northwest production belt
- **EZ/EPZs nearby:** Sirajganj EZ, Panchagarh EZ, Ishwardi EPZ
- **Ports nearby:** Banglabandha, Burimari, Mongla Port, Hili, Sonarhat
- **RCA:** Jute raw fibres +16.71, jute fabrics +4.14, spices +0.29, oilseeds +0.30; jute yarn –2.69
- **Quick wins in:** Agribusiness (potatoes, maize, jute, cold storage)
- **Existing Operations:** PRAN-RFL Agro factory (Natore), PRAN-RFL Habiganj Industrial Park (North-East), Giant Agro Processing Ltd (Bhaluka), Apex Foods (Rupsha, Khulna).
- **Recommendation(s):** Establish Multimodal Logistics Hubs (MMLH) and cold storage to address strong seasonality and reliance on medium trucks for long-haul without consolidation points.
- **Note:** Agricultural heartland with massive backward linkages (rice 30.29, aquaculture 30.17).

## 8. IT-enabled Services (IT-ES)
- **Cluster(s):** North Dhaka Cluster, Mirsharai Cluster, North-East (Sylhet)
- **Belt(s):** Central Belt
- **EZ/EPZs nearby:** Hi-Tech City Kaliakair, Sylhet HiTech Park, Rajshahi HiTech Park
- **Ports nearby:** Not applicable for digital services (relies on submarine cable landing stations like Cox's Bazar and Kuakata; nearest physical seaport is Chattogram at 250+ km from Dhaka)
- **RCA:** ICT sector total linkage 22.00 (identified as high-complexity, high GVC-integration opportunity)
- **Quick wins in:** BPO, freelance outsourcing, SaaS, AI data annotation, technology villages
- **Existing Operations:** Hi-Tech City Kaliakair (Operational data centers/IT firms), Sylhet Hi-Tech Park (IT incubation firms), BEPZA Economic Zone (Infrastructure operational in BSMSN).
- **Recommendation(s):** Pivot to 1M tech jobs, semiconductor development, AI data center campus.
- **Note:** Leverages Dhaka's university ecosystem and skilled labor. Large worker population in Mirsharai creates demand for private health/education providers.

## 9. Light Engineering
- **Cluster(s):** North Bengal Cluster, South-West Cluster, Southeast Cluster
- **Belt(s):** Central Belt
- **EZ/EPZs nearby:** Bogura production zones
- **Ports nearby:** Mongla Port, Chattogram Port
- **RCA:** Spectacle frames +0.13, clock movements +0.49
- **Quick wins in:** Light engineering, creative industries (SMEs)
- **Existing Operations:** Bogura Light Engineering Cluster (BSCIC Industrial Estate, ~800 operational workshops), Jashore BSCIC Light Engineering workshops.
- **Recommendation(s):** Link with agro-processing in Bogura and port services in Chattogram.
- **Note:** Machinery/equipment sector total linkage 7.20; nascent but growing.

## 10. Renewable Energy
- **Cluster(s):** Moheshkhali Cluster
- **Belt(s):** Southeast gateway belt
- **EZ/EPZs nearby:** Moheshkhali EZ, Matarbari hub
- **Ports nearby:** Matarbari Deep-Sea Port
- **RCA:** Electricity/gas/steam sector total linkage 23.36
- **Quick wins in:** Green ship recycling
- **Existing Operations:** Teknaf Solar Power Plant (Joules Power, 20MW operational in Cox's Bazar), Matarbari Coal Power Plant (operational but transitioning energy focus).
- **Recommendation(s):** Develop climate-resilient infrastructure to mitigate extreme cyclone and sea-level vulnerability.
- **Note:** National strategic priority — heavy industry and energy hub with 3,500+ acres of deep-water access. Energy & Power receives 22.3% ($17.7B) of total corridor investment.

## 11. Logistics
- **Cluster(s):** Chattogram Cluster, North Bengal Cluster
- **Belt(s):** Southeast gateway belt, Northwest production belt
- **EZ/EPZs nearby:** All major EPZs/EZs
- **Ports nearby:** Seaports (Chattogram, Mongla, Payra, Matarbari); Land ports (Benapole, Bhomra, Tamabil, Akhaura, Bibirbazar, Nakugaon, Banglabandha, Burimari)
- **RCA:** Transport & Storage total linkage 39.28; wholesale/retail trade 57.96 total linkage
- **Quick wins in:** Port-centric logistics, cross-border logistics
- **Existing Operations:** Summit Alliance Port Limited (SAPL, Chattogram), Matarbari Deep-Sea Port channel, Banglabandha & Tamabil Land Ports (Operational cross-border trade).
- **Recommendation(s):** Develop Multimodal Logistics Hubs (MMLH) as consolidation, intermodal transfer, cold-chain, and bonded operations platforms. Shift from road haulage to rail/IWT.
- **Note:** 200–300 km trip band is most common for freight. Strong transshipment potential for Nepal and Bhutan; could evolve into BIMSTEC/BBIN transshipment route.
